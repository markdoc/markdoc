...
function parse(
  content: string,
  { file, tokenizerSettings }: { file?: string; tokenizerSettings?: any }
) {
  const t = tokenizerSettings
    ? new markdoc.Tokenizer(tokenizerSettings)
    : tokenizer;
...
function render(code, config, dynamic) {
  ...
  return dynamic
    ? react(transformed, React, {})
    : eval(reactStatic(transformed));
}
